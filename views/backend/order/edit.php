...
<?php 
  ...
  $id=$_REQUEST['id'];
  $list_order = order::where([['status','!=', '0'],['id','=', $id]])
  ->get();
  $html_category_id_order="";
  foreach($list_order as $item){
    $html_category_id_order = $item->category_id;
  }

  $html_brand_id_order="";
  foreach($list_order as $item){
    $html_brand_id_order = $item->brand_id;
  }


  $list_category = Category::where([['status','!=', '0'],['id','=', $html_category_id_order]])
  ->get();
  $list_brand = Brand::where([['status','!=', '0'],['id','=', $html_brand_id_order]])
  ->get();
  //category
  $html_category_id="";
