...
<?php 
  ...
  $id=$_REQUEST['id'];
  $list_category = Category::where([['status','!=', '0'],['id','=', $id]])
  ->get();
        ...
        <section class="content">    
                <form action="index.php?option=category&cat=process" method="post" enctype="multipart/form-data">                      
                    ...
                    <select name="parent_id" id="parent_id" class="form-control">
                        <?php foreach($list_category as $item):?>
                            <option value="<?=$item['id'];?>">Hiện tại: <?=$item['name'];?></option>
                        <?php endforeach;?>
                        <option value="0">Đổi: danh mục cha</option>
                        <?php foreach($list_categoryAll as $item1):?>
                            <option value="<?=$item1['id'];?>">Đổi: <?=$item1['name'];?></option>
                        <?php endforeach;?>
                    </select>
                    </div>
                    ...
                    <?php foreach($list_category as $item):?>
                      <img src="../public/img/category/<?=$item['img'];?>" alt="anh" class="imageEdit"> 
                        <?php endforeach;?><br>                   
